...
/** `message` is rendered from `login_msg.md` by the server component. */
export default function LoginForm({ message }: { message?: ReactNode }) {
  ...
  if (mode === "loading") {
    // The message comes from the server, so it is already in the HTML — keep it
    // on screen while the auth status resolves instead of flashing it in later.
    return (
